...
class GraphReadOnlyConnector(ReadOnlyMailConnector):
    ...
    def _build_app(self):
        ...
        if not authority:
            raise ConnectorNotAvailableError("OAuth authority must be provided for Microsoft Graph authentication.")
    ...
    def _ensure_token(self) -> str:
        ...
        if "access_token" not in token:
            raise ConnectorNotAvailableError(f"Unable to obtain Microsoft Graph access token: {token.get('error')}")
        self._token = token
        self._token_acquired_at = now
        return token["access_token"]

    def _convert_message_summary(self, payload: dict[str, Any], fallback_folder: str | None) -> MessageSummary:
        ...
        summary = MessageSummary(
            ...
            from_=parse_address_list(_extract_address(payload.get("from"))),
